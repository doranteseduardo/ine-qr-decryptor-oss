/* ══ FILE: crypto_backend_boringssl.c ══
 *
 * BoringSSL implementation of ine_crypto_backend.h for the Android NDK build.
 *
 * BoringSSL is API-compatible with the OpenSSL surface no_so_crypto.c uses
 * for AES (EVP_*) and RSA bignum (BN_*). Base64 is handled by the vendored
 * base64.c — BoringSSL has been deprecating the BIO_f_base64 path so we
 * sidestep that surface entirely.
 *
 * The body of this file is the exact same code as
 * ine-qr-c/src/crypto_backend_openssl.c — different name only because Android
 * Studio surfaces it that way in the project tree. Keep the two in sync;
 * any change here should also land in the OpenSSL backend.
 */
