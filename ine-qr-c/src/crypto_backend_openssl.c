 ...
 *
 * Shared by the desktop ine_decode CLI, libine_decode.a (cgo Go API), and the
 * Android NDK build (which links the same source against BoringSSL's libcrypto
 * — every OpenSSL API used here exists in BoringSSL with identical signatures).
 *
 * The four primitives are exactly what no_so_crypto.c calls. They were lifted
 * out of static_crypto.c verbatim during the mobile-port refactor; static_
 * crypto.c now keeps only the emulator-specific run_static_pipeline() and
 * precompute_buf2() helpers.
 */
