...
`ine-qr-py/no_so_pipeline.py`, and `ine-qr-wasm/src/pipeline.c`. The
emulator path (`ine-qr-c/src/main_emulator.c` + `ine-qr-py/emulate_pc.py`)
runs the original `.so` and produces the ground-truth captures used to
verify the pure paths.

For the Unicorn-sandbox details see [EMULATOR.md](EMULATOR.md). For the
historical RE narrative see [REVERSE_ENGINEERING.md](REVERSE_ENGINEERING.md).
...
CT_R1_KEY  @ .rodata 0x1182cd  (1456 bytes, AES-encrypted RSA-XML #1)
CT_R1_BLOB @ .rodata 0xd05ae   (2736 bytes, AES-encrypted base64 RSA blob)
CT_R2_KEY  @ .rodata 0x107e31  (1456 bytes, AES-encrypted RSA-XML #3)
CT_R2_BLOB @ .rodata 0xee12e   (2736 bytes, AES-encrypted base64 RSA blob)

WebP header table @ .rodata 0x124b10  (23 bytes: RIFF / WEBP / VP8 / 9D 01 2A …)
```
