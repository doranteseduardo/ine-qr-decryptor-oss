...
### `ine-qr-c/` — C/C++ reference
```bash
cd ine-qr-c
make            # pure-C CLI (ine_decode)
make lib        # libine_decode.a (static archive for FFI embedding)
```
...
### `ine-qr-py/` — Python
```bash
python3 ine-qr-py/no_so_pipeline.py <photo>   # pure-Python reference
```
Deps: `cryptography numpy Pillow zxing-cpp`.
...
### `ine-qr-eskiu/` — Eskiu implementation
```bash
cd ine-qr-eskiu
make test       # crypto+output parity (needs eskiuc on PATH — see its README)
```
