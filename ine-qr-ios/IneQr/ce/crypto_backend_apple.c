 ...
 * signature; no wrapper required. base64_encode is only used by the
 * emulator path (static_crypto.c), which is not shipped on iOS. Stub it
 * so the symbol resolves if any future caller links against it. */
